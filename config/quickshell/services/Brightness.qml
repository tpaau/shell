...
Singleton {
	...
	readonly property real brightnessDelta: 0.1
    ...
    function getMonitor(query: string): var {
        ...
        if (query.startsWith("id:")) {
            ...
            return monitors.find(m => Niri.outputFromShellScreen(m.modelData)?.id === id)
        }
    ...
    function increaseBrightness(): void {
		...
		if (monitor) {
            monitor.setBrightness(monitor.brightness + brightnessDelta)
		} else {
			console.warn("No active monitor was found")
		}
    }
    ...
    component Indicator: ToastProgressIndicator {
        required property Monitor monitor

        icon: Icons.pickIcon(monitor.brightness, ["", "", ""])
        progress: monitor.brightness
    ...
    component Monitor: Item {
        ...
        onBrightnessChanged: {
            Ipc.displayIndicatorToast(indicator)
        }
        ...
        function setBrightness(value: real): void {
			if (!initialized) {
				console.warn("Monitor not initialized, dropping brightness set operation")
				return
			}
