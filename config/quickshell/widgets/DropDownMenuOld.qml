...
StyledButton {
	...
	property DropDownMenuEntryOld selected: {
	...
	required property list<DropDownMenuEntryOld> entries

	signal picked(entry: DropDownMenuEntryOld)
	function pick(entry: DropDownMenuEntryOld) {
		expanded = false
		selected = entry
		picked(entry)
	}
			...
			ColumnLayout {
				...
				Repeater {
					...
					StyledButton {
						...
						readonly property DropDownMenuEntryOld model:
